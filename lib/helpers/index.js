...
export const getCurrencyRates = async (code) => {
  const data = await fetch(
    process.env.NODE_ENV === "development"
      ? `${process.env.NEXT_PUBLIC_DEV_URL}/api/currency?code=${
          !code ? "USD" : code
        }`
      : `${process.env.NEXT_PUBLIC_PUBLIC_URL}/api/currency?code=${
          !code ? "USD" : code
        }`
  );
