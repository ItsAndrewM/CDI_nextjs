...
import { CurrencyProductPage } from "./currencyProductPage";
...
export function ProductPage({ product }) {
  ...
  const [shippingClass, setShippingClass] = useState(() => {
    if (cart) {
      const shippingClass = cart.meta_data.find(
        (item) => item.key === "shipping_class"
      );
  ...
  const handleAddToCart = async (e) => {
    e.preventDefault();
    try {
      if (selected || product.type === "simple") {
        setLoading(true);
            ...
            : product.images.map((image, index) => {
                if (index > 0) {
                  return (
                    <Image
                      alt={image.alt}
                      className="aspect-square object-cover border border-gray-200 w-full max-w-full rounded-lg overflow-hidden "
                      height="200"
                      key={image.id}
                      src={image.src}
                      width="200"
      ...
      <div className="grid gap-4 md:gap-10 items-start">
        <div className="grid gap-4">
          ...
          <div className="text-4xl font-bold">
            {`${currentRate.code} ${product.prices.currency_symbol} ${
              selected
                ? (currentRate.rate * Number(selected.price)).toFixed(2)
                : (currentRate.rate * Number(product.price)).toFixed(2)
            }`}
          </div>
        </div>
